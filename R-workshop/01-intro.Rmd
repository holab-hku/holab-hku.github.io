...
-  Download the data from the GitHub:
  [holab-hku.github.io/R-prework/data/](https://github.com/holab-hku/holab-hku.github.io/tree/master/R-prework/data) for this workshop and store the data in the "Data" folder
...
[The original data source](https://figshare.com/s/1d788fd384d33e913a2a)
...
```
## Filtering 
Then we can filter out some lowly expressed genes. There are two sample replicates for each group, so we can filter on the RPM with some threshold present in at least 2 samples. 
And how to decide your threshold, normally a RPM corresponding to count of 10-15 is a good threshold. We can use the scatter plot to decide the number of RPM corresponding to the count 10.
```{r}
...
```
As we can see RPM = 0.5 can be a very good threshold. Then let's summarize how many RPM greater than 0.5 there are in each row
```{r}
...
We can see that the boxplot before normalization looks not good.
```{r}
logcounts <- as.matrix(myRPM.keep)
boxplot(logcounts, xlab="", ylab="counts per million")
title("Boxplots of myRPM(Unnormalized)")
```
...
Principle components analysis, which determines the greatest sources of variation in the data. We can use the [prcomp()](https://www.rdocumentation.org/packages/stats/versions/3.6.2/topics/prcomp) function to perform a principal components analysis on the given data matrix from the package *stats*.
...
library(ggfortify)
```
First, we create the right inpur format for ``stats::prcomp``, and we add a new column called CellType with the cell type information from sampleinfo.
```{r warning=TRUE}
